...
export function SairApp() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      void loadBatchDetail(currentBatchId, "current");
      void requestJson<{ batches: RunBatchSummary[] }>("/api/runs").then((payload) =>
        setBatches(payload.batches),
      );
    }, 1500);

    return () => window.clearInterval(interval);
  }, [currentBatch, currentBatchId, loadBatchDetail]);
        ...
        <div className="layout">
          <aside className="sidebar">
            ...
            <button
              ...
              onClick={submitRun}
            >
              {submittingRun ? "Launching..." : `Run ${selectedCount} problem${selectedCount === 1 ? "" : "s"}`}
            </button>
          </aside>
          ...
          <section className="workspace">
            {tab === "run" ? (
                  ...
                  <div className="run-panel">
                    <div className="panel panel--main">
                      <div className="panel__header">
                        ...
                        <div className={`status-pill status-pill--${currentBatch.status}`}>
                          {currentBatch.status}
                        </div>
                      </div>
                      ...
                      {currentBatch.items.map((item) => (
                        <article key={item.id} className="result-card">
                          ...
                          <details className="details-block">
                            ...
                            <pre>{item.error ?? item.rawResponse ?? ""}</pre>
                          </details>
              ...
              <div className="history-layout">
                <section className="panel panel--history-list">
                    ...
                    {historyBatches.map((batch) => (
                      ...
                      >
                        ...
                        <div className="history-row__meta">
                          ...
                          <span className={`status-pill status-pill--${batch.status}`}>
                            {batch.status}
                          </span>
                        </div>
                      </button>
                  ...
                  {!historyBatch ? (
                    ...
                    <>
                      <div className="panel__header">
                        ...
                        <div className={`status-pill status-pill--${historyBatch.status}`}>
                          {historyBatch.status}
                        </div>
                      </div>
      ...
      {draft ? (
        <div className="modal-backdrop" role="presentation" onClick={() => setDraft(null)}>
          ...
          >
            <div className="panel__header">
              <div>
                <p className="eyebrow">Cheatsheet</p>
                <h2>{draft.id ? "Edit saved context" : "Create saved context"}</h2>
              </div>
            </div>
            ...
            <textarea
              className="textarea"
              placeholder="Paste reusable lemmas, proof patterns, or reminders here."
              value={draft.content}
              onChange={(event) =>
                setDraft((current) =>
            ...
            <div className="modal__actions">
              ...
              <button
                ...
                disabled={!draft.name.trim() || savingCheatsheet}
                onClick={saveCheatsheet}
              >
                {savingCheatsheet ? "Saving..." : "Save cheatsheet"}
              </button>
            </div>
          </div>
        </div>
